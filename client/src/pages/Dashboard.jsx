...
import {
  ...
  MdEdit, // Replaced LuClipboardEdit with MdEdit
} from "react-icons/md";
    ...
    <tr className="border-b border-gray-300 text-gray-600 hover:bg-gray-300/10">
      ...
      <td className="py-2 hidden md:block">
        <span className="text-base text-gray-600">
          {moment(task?.date).fromNow() || "No date provided"}
        </span>
      </td>
    </tr>
  );
  ...
  const TableRow = ({ user }) => (
    <tr className="border-b border-gray-200 text-gray-600 hover:bg-gray-400/10">
      <td className="py-2">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-full text-white flex items-center justify-center text-sm bg-violet-700">
            <span className="text-center">{getInitials(user?.name || "NA")}</span>
          </div>
          <div>
      ...
      <td className="py-2 text-sm">
        {moment(user?.createdAt).fromNow() || "Unknown date"}
      </td>
    </tr>
  );
...
const Dashboard = () => {
  ...
  const stats = [
    ...
    },
    {
      ...
      icon: <MdEdit />, // Corrected icon here
      bg: "bg-[#f59e0b]",
    },
    {
      _id: "4",
